...
- **Consumption pattern (from apps/storybook):** deps `@handamade/tokens`/`@handamade/react` `workspace:*` + react/react-dom `^19.2.0`; import `base.css`, theme css files, `utilities.css`, `components.css`, `@handamade/react/styles`; packages resolve from built `dist/`.
...
Roadmap items worth an honest "What's next" section: custom listbox Select (v2), Popover-API Tooltip (v2), `@handamade/mcp` server (v2 candidate), visual regression testing.
...
- CSS import order in `main.tsx`: tokens `base → light → dark → acme → utilities → components` → `@handamade/react/styles` → local `promo.css`.
...
- No new components added to `@handamade/react`; all composition stays in the app (matches spec's v1 philosophy). If layout/typography gaps hurt repeatedly, promote them to tokens/components later as a separate effort.
